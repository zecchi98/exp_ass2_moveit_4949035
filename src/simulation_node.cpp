...
#include <exp_ass2_moveit_4949035/ErlOracle.h>
...
std::vector<exp_ass2_moveit_4949035::ErlOracle> oracle_msgs;
...
bool oracleService(exp_ass2_moveit_4949035::Oracle::Request &req, exp_ass2_moveit_4949035::Oracle::Response &res)
	{
		res.ID = winID;
		return true;
	}
   ...
   for(int i=0; i< msg->name.size(); i++){
	   if (msg->name[i].find("cluedo_link")!= std::string::npos){
		   for(int j=0; j<4;j++){
			    ...
			    double distanza=distfromtarget(msg->pose[i].position.x, msg->pose[i].position.y, msg->pose[i].position.z, markx[j],marky[j],markz[j]);
				//if(j==0) ROS_INFO("Distanza:%f",distanza);
...
oracle_pub = nh.advertise<exp_ass2_moveit_4949035::ErlOracle>( "/oracle_hint", 0 );
